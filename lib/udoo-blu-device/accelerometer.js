...
AccelerometerSensor.prototype.subscribeDetectionAccLin = function (udooblu, xEnable, yEnable, zEnable, threshold, callback, subscribed) {
    ...
    async.series([
        ...
        function (asyncCallback) {
            that.sensor.forcePeriodToZero(udooblu, AccelerometerServiceUUID, ()=>{})
            setTimeout(asyncCallback, 2000);
        },
        function (asyncCallback) {
            udooblu.writeUInt8Characteristic(AccelerometerServiceUUID, AccelerometerDetectionTypeUUID, 0x01, ()=>{});
            setTimeout(asyncCallback, 2000);
        },
        function (asyncCallback) {
            udooblu.writeUInt8Characteristic(AccelerometerServiceUUID, AccelerometereDetectionFlagsUUID, dtFlags, ()=>{});
            setTimeout(asyncCallback, 2000);
        },
        function (asyncCallback) {
            udooblu.writeUInt16LECharacteristic(AccelerometerServiceUUID, AccelerometerDetectionThresholdUUID, threshold, ()=>{});
            setTimeout(asyncCallback, 2000);
        },
        function (asyncCallback) {
            that.sensor.on('accelerometerSensorChange', subscribed);
